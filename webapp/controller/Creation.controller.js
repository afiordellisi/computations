sap.ui.define(
  ...
  function (
    ...
    return BaseController.extend(
      "tax.provisioning.computations.Creation.controller.Creation",
      {
        formatter: formatter,
        onInit: function () {
        ...
        filterValidation: function () {
          ...
          if (societa && periodo && ledger && descrizione) {
            ...
            if (!societa) this.getView().byId("societa").setValueState("Error");
            if (!ledger) this.getView().byId("ledger").setValueState("Error");
            if (!periodo) this.getView().byId("periodo").setValueState("Error");
            if (!descrizione)
        ...
        filterValidation2: function (oEvent) {
          ...
          if (sSelectedPath != null) {
            var sSelectedPath = oTable
              .getSelectedItem()
              .getBindingContext("tableModel")
              .getPath()
              .split("/oModel/")[1];
            var oItem = oTable.getSelectedItem().getModel("tableModel").oData
              .oModel[sSelectedPath];

            var versioneID = oItem.ID;

            this.getView().getModel(
              "parameterModel"
            ).oData.idVersione = versioneID;
            var aFilter = [];
            aFilter.push(
              this.getView().getModel("parameterModel").oData.societa
            );
            aFilter.push(
              this.getView().getModel("parameterModel").oData.ledger
            );
            aFilter.push(
              this.getView().getModel("parameterModel").oData.periodo
            );
        ...
        filterValidation3: function (oEvent) {
          ...
          if (sSelectedPath != null) {
            var sSelectedPath = oTable
              .getSelectedItem()
              .getBindingContext("taxRuleModel")
              .getPath()
              .split("/oModel/")[1];
            var oItem2 = oTable.getSelectedItem().getModel("taxRuleModel").oData
              .oModel[sSelectedPath];
            this.getView().getModel("parameterModel").oData.idConfigurazione =
              oItem2.ID;
        ...
        filterValidation4: function (oEvent) {
          ...
          var checkIRES = modelloIRES.some((element) =>
            Object.values(element).some((val) => val <= 0 || val >= 100)
          );
          var checkIRAP = aIRAPNOImponibile.some((element) =>
            Object.values(element).some((val) => val <= 0 || val >= 100)
          );
        ...
        _filterTableTax: function () {
          ...
          jQuery.ajax({
            url: jQuery.sap.getModulePath(
              sap.ui.getCore().sapAppID + "/catalog/Regions"
            ),
            ...
            success: function (oCompleteEntry) {
              var dataIRES = oCompleteEntry.value.filter(function (oItem) {
                return oItem.imposta === "IRES";
              });
              ...
              var dataIRAP = oCompleteEntry.value.filter(function (oItem) {
                return oItem.imposta === "IRAP";
              });
        ...
        handleFinishSelection: function () {
          ...
          if (sSelectedPath != null) {
            var sSelectedPath = oTable
              .getSelectedItem()
              .getBindingContext("confCompModel")
              .getPath()
              .split("/oModel/")[1];
            var oItem2 = oTable.getSelectedItem().getModel("confCompModel")
              .oData.oModel[sSelectedPath];
            this.getView().getModel("parameterModel").oData.idCompConfronto =
              oItem2.ID;
        ...
        handleWizardSubmit: function (oEvent) {
          if (this.handleFinishSelection()) {
            var that = this;
            var descrizioneComputation = this.getView().getModel(
              "parameterModel"
            ).oData.descrizione;
            var idConfigurazione = this.getView().getModel("parameterModel")
              .oData.idConfigurazione;
            var idVersione = this.getView().getModel("parameterModel").oData
              .idVersione;
            var idCompConfronto = this.getView().getModel("parameterModel")
              .oData.idCompConfronto;
            ...
            var nuovaComputation = JSON.stringify({
              descrizione: descrizioneComputation,
              Configurazione_ID: idConfigurazione,
              Versione_ID: idVersione,
              compConfronto: idCompConfronto,
              confRegions: confRegions,
            });
        ...
        onDialogNextButton: function () {
          ...
          if (
            stepName ===
            "application-taxprovisioningcomputations-display-component---Creation--trialBalance"
          ) {
          ...
          if (
            stepName ===
            "application-taxprovisioningcomputations-display-component---Creation--taxRule"
          ) {
          ...
          if (
            stepName ===
            "application-taxprovisioningcomputations-display-component---Creation--confConfronto"
          ) {
          ...
          this._oSelectedStep = this._oWizard.getSteps()[
          ...
          this._iSelectedStepIndex = this._oWizard
            .getSteps()
            .indexOf(this._oSelectedStep);
          var oNextStep = this._oWizard.getSteps()[
            this._iSelectedStepIndex + 1
          ];
        ...
        _filterTableConf: function (aFilter) {
          ...
          jQuery.ajax({
            url: jQuery.sap.getModulePath(
              sap.ui.getCore().sapAppID +
                "/catalog/Configurazioni?$filter=societa eq '" +
                societa +
                "' and ledger eq '" +
                ledger +
                "' and periodo eq '" +
        ...
        _filterTableCompConfronto: function (aFilter) {
          ...
          jQuery.ajax({
            url: jQuery.sap.getModulePath(
              sap.ui.getCore().sapAppID +
                "/catalog/Computations?$expand=Versione&$filter=Versione/periodo eq '" +
                periodo +
                "' and Versione/societa eq '" +
                societa +
                "' and Versione/ledger eq '" +
                ledger +
                "' "
            ), //filtro solo per la tripletta
            // url: jQuery.sap.getModulePath(sap.ui.getCore().sapAppID + "/catalog/Configurazioni?$filter=versione_ID eq "+versioneID+"&societa eq '"+societa+"'&ledger eq '"+ledger+"'&periodo eq '"+periodo+"'"), //qui filtro anche per versione
